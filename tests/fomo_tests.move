#[test_only]
module fomo::fomo_tests {
    use sui::bcs;
    use sui::balance;
    use sui::clock;
    use sui::test_utils::assert_eq;
    use sui::coin::{Self, Coin};
    use fomo::locker as tlb;
    use fomo::miner::{register, Miner};
    use fomo::fomo::{
    ...
    #[test]
    fun test_hash() {
        let r2 = fomo::generate_proof_pub(mock_hash(), SIGNER, NONCE_DIFFICULTY_2);
        fomo::validate_proof_pub(r2, 2);
        let r3 = fomo::generate_proof_pub(mock_hash(), SIGNER, NONCE_DIFFICULTY_3);
        fomo::validate_proof_pub(r3, 3);
        let r4 = fomo::generate_proof_pub(mock_hash(), SIGNER, NONCE_DIFFICULTY_4);
        fomo::validate_proof_pub(r4, 4);
    }

    #[test, expected_failure(abort_code = fomo::EInsufficientDifficulty)]
    fun test_mock_hash_fail() {
        let res = fomo::generate_proof_pub(mock_hash(), SIGNER, MOCK_NONCE);
        fomo::validate_proof_pub(res, 1);
    }
    ...
    fun test_insufficient_diff_fail() {
        let r4 = fomo::generate_proof_pub(mock_hash(), SIGNER, NONCE_DIFFICULTY_4);
        fomo::validate_proof_pub(r4, 4);
        fomo::validate_proof_pub(r4, 5);
    }

    #[test]
    public fun test_difficulty_grading() {
        use fomo::fomo::calculate_difficulty_pub as calc;

        let initial_difficulty = fomo::get_initial_difficulty();
        let total_supply = fomo::get_supply();
    ...
    public fun test_reset() {
        ...
        clock.set_for_testing(1727883901000); // after start time
        clock.set_for_testing(1727883901000 + fomo::get_epoch_duration() + ONE_DAY);

        let bus_epoch_rewards = fomo::get_bus_epoch_rewards();

        test.next_tx(SIGNER);
        {
            let ctx = test.ctx();
            fomo::init_for_testing(ctx);
        };
    ...
    #[test, expected_failure(abort_code = fomo::EInsufficientBuses)]
    ...
    #[test, expected_failure(abort_code = fomo::EMiningHasEnded)]
    ...
    fun create_endgame(signer: address, test: &mut sui::test_scenario::Scenario, clock: &mut clock::Clock) {
        ...
        // ADVANCE CLOCK
        {
            ...
            let burn = fomo::simulate_claims(&mut miner, &mut config, clock);
        ...
        while (!exit) {
            ...
            clock.increment_for_testing(fomo::get_epoch_duration() + 1);
    ...
    fun get_buses(scenario: &sui::test_scenario::Scenario): vector<Bus> {
        let mut xs = vector[];
        while (xs.length() < fomo::get_initial_bus_count()) {
    ...
    fun assert_value(coin: Coin<fomo::FOMO>, value: u64) {
